...
public class MailServiceClient {
    public static void main(String[] args) throws MalformedURLException, WebStateException {
        ...
        List<Attachment> attachments = ImmutableList.of(new Attachment(
                "version.html",
        ...
        String state = mailService.sendToGroup(
                ImmutableSet.of(new Addressee("sane4ever@ya.ru", null)),
                null, "Group mail subject", "Group mail body", attachments
        );
        System.out.println("Group mail state: " + state);

        GroupResult groupResult = mailService.sendBulk(
                ImmutableSet.of(
                        new Addressee("Мастер Java <sane4ever@ya.ru>"),
                        new Addressee("Bad Email <bad_email.ru>")),
                "Bulk mail subject", "Bulk mail body", attachments
        );
