...
public class AkkaActivator {
    ...
    public <T> void startTypedActor(Class<T> typedClass, String name, Creator<T> creator) {
        log.info("Start AKA typed actor: {}", name);
        TypedActor.get(system).typedActorOf(new TypedProps<T>(typedClass, creator)
                .withTimeout(new Timeout(Duration.create(20, SECONDS))));
    }
