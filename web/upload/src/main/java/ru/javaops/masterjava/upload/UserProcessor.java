    ...
    public List<PayloadProcessor.FailedEmails> process(
        ...
        chunkFutures.forEach((emailRange, future) -> {
            try {
                ...
                failed.add(new PayloadProcessor.FailedEmails(emailRange, e.toString()));
            }
        });
        if (!allAlreadyPresents.isEmpty()) {
            failed.add(new PayloadProcessor.FailedEmails(allAlreadyPresents.toString(), "already presents"));
        }
        return failed;
    }
    ...
    private Map<String, Future<List<String>>> createChunkFutures(
        ...
        while (processor.doUntil(XMLEvent.START_ELEMENT, "User")) {
            ...
            if (cities.get(cityRef) == null) {
                ...
                List<String> groupNames = groupRefs == null ? Collections.emptyList() : Splitter.on(' ').splitToList(groupRefs);
                if (!groups.keySet().containsAll(groupNames)) {
                    ...
                    chunk.add(user);
                    List<UserGroup> userGroups = StreamEx.of(groupNames)
                            .map(name -> new UserGroup(user.getId(), groups.get(name).getId()))
                            .toList();
                    chunkUserGroups.addAll(userGroups);
                    if (chunk.size() == chunkSize) {
                        addChunkFutures(result, chunk, chunkUserGroups);
        ...
        Future<List<String>> future = executorService.submit(() -> {
            ...
            List<String> alreadyPresentEmails = DBIProvider.getDBI().inTransaction((handle, status) -> {
                ...
                tUserGroupDao.insertBatch(
                        StreamEx.of(chunkUserGroups).filter(ug -> !alreadyPresentsIds.contains(ug.getUserId())).toList()
                );
