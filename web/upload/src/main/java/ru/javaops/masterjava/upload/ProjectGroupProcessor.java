...
public class ProjectGroupProcessor {
    ...
    public Map<String, Group> process(StaxStreamProcessor processor) throws XMLStreamException, JAXBException {
        ...
        Projects projects = jaxbParser.createUnmarshaller().unmarshal(processor.getReader(), Projects.class);
        projects.getProject().forEach(p -> {
            ...
            p.getGroup().forEach(group -> {
                if (!groupMap.containsKey(group.getName())) {
                    newGroups.add(new Group(group.getName(), GroupType.valueOf(group.getType().value()), projectId));
                }
            });
        });
